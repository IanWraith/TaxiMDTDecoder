...
/**
 * @author Andy
 * Class to handle the various packet types
 */
public class Decoder {
	...
	public Decoder(TaxiMdtDecoder theApp){
		this.theApp = theApp;
	}
	...
	public String[] handle_autocab(boolean bit, int system_code) {
		...
		if (message_length == bits_rxed) {
			...
			// Decode the data //
			autocab_object.decode(raw_bits, theApp.sysInfo);
			clear_ready_for_next_msg();
			// Message filtering //
			int messageType=autocab_object.getMessageType();
			int messageSubType=autocab_object.getMessageSubType();
			// Type 0
			if ((view_autocab_t0_ack == false)
					&& (messageType == 0)
					&& (messageSubType == 0))
				return null;
			if ((view_autocab_t0_bcast == false)
					&& (messageType == 0)
					&& ((messageSubType == 10) || (messageSubType == 42)))
				return null;		
			if ((view_autocab_t0_s132 == false)
					&& (messageType == 0)
					&& (messageSubType == 132))
				return null;			
			// Type 1
			if ((view_autocab_t1 == false)
					&& (messageType == 1))
				return null;		
			// Type 4
			if ((view_autocab_t4_s1 == false)
					&& (messageType == 4)
					&& (messageSubType == 1))
				return null;			
			if ((view_autocab_t4_s6 == false)
					&& (messageType == 4)
					&& (messageSubType == 6))
				return null;						
			if ((view_autocab_t4_s32 == false)
					&& (messageType == 4)
					&& (messageSubType == 32))
				return null;			
...
//			display_view.add_array(autocab_object.line);
	...
	public String[] handle_auriga(boolean bit) {
		...
		if ((raw_bits.get(bits_rxed - 1) == false)
				&& (raw_bits.get(bits_rxed - 2) == true)
				&& (raw_bits.get(bits_rxed - 3) == true)
				&& (raw_bits.get(bits_rxed - 4) == true)
				&& (raw_bits.get(bits_rxed - 5) == true)
				&& (raw_bits.get(bits_rxed - 6) == true)
				&& (raw_bits.get(bits_rxed - 7) == true)
				&& (raw_bits.get(bits_rxed - 8) == false)) {
			...
			// Decode the data //
			auriga_object.decode(unstuffed_data, theApp.sysInfo);
	...
	public String[] handle_autocab_e(boolean bit) {
		...
		if (bits_rxed == (message_length + 1)) {
			...
			if (message_length == 128) {
				int tlen = 0;
				if (raw_bits.get(16) == true)
					tlen = 128;
				if (raw_bits.get(17) == true)
					tlen = tlen + 64;
				if (raw_bits.get(18) == true)
					tlen = tlen + 32;
				if (raw_bits.get(19) == true)
					tlen = tlen + 16;
				if (raw_bits.get(20) == true)
					tlen = tlen + 8;
				if (raw_bits.get(21) == true)
					tlen = tlen + 4;
				if (raw_bits.get(22) == true)
					tlen = tlen + 2;
				if (raw_bits.get(23) == true)
					tlen++;
			...
			// Decode the data //
			autocab_e_object.decode(raw_bits, theApp.sysInfo);
			// Display the info //
...
//			clear_ready_for_next_msg();
	...
	public void decode(boolean in, int whichChannel) {
		...
		if (decodedMsg != null){
			this.theApp.addArray(decodedMsg, whichChannel);
			clear_ready_for_next_msg();
		}
	}
