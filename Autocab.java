...
//				if (messageSubType == 10)handle_autocab_t0_bcast_s10(rawData);
				if (messageSubType == 10)handle_autocab_t0_bcast_s42(rawData);
				if (messageSubType == 42)handle_autocab_t0_bcast_s42(rawData);
			}
			// Other
			else 
...
/*		for (i = 2; i < (messageBytes - 3); i = i + 1) {
		...
		if ((fourBit[0]>0)||(fourBit[1]>0)||(fourBit[2]>0)||(fourBit[3]>0))
		 {
		 line[lineNumber]=String.format("Plot %03d - Cars %02d : Jobs Now %02d : Jobs 15 Mins %02d : Jobs 30 Mins %02d",messageByte(bits,i),fourBit[0],fourBit[1],fourBit[2],fourBit[3]);
	...
	private String handle_autocab_t0_nonbcast(ExBitSet bits) {
		...
		if (mtype == 148) {
			...
			for (a=1; a<(messageBytes); a=a+1)	{
				...
				if ((c>47) && (c<58)) tmp.append(Character.toString((char) c));
			}			
		}
		...
		if (mtype == 242) {
			...
			// Status meanings
			if (val.equals("00")) tmp.append("Recall Last Message");
			if (val.equals("77")) tmp.append("Soon to Clear");
			if (val.equals("78")) tmp.append("Long Break");
			if (val.equals("82")) tmp.append("Short Break");
			if (val.equals("81")) tmp.append("Finish Break");
			if (val.equals("86")) tmp.append("Yes");
			if (val.equals("87")) tmp.append("No");
			if (val.equals("90")) tmp.append("Voice Request");
			if (val.equals("91")) tmp.append("Information");
			if (val.equals("92")) tmp.append("Direction");
			if (val.equals("93")) tmp.append("Finish Work for the Day");
			if (val.equals("94")) tmp.append("Waiting Outside");
			if (val.equals("95")) tmp.append("Clear");
			if (val.equals("96")) tmp.append("Ringback");
			if (val.equals("97")) tmp.append("Recover Job");
			if (val.equals("98")) tmp.append("No Job");
			if (val.equals("83")) tmp.append("Bid");
			if (val.equals("88")) tmp.append("Mobile");
			if (val.equals("99")) tmp.append("Emergency");
		}
...
/*        // Display the message
	...
	private String handle_autocab_t4(ExBitSet bits) {
		...
		if (messageByte(bits,messageBytes-19) == 0) {
			if (getViewLinks() == true)	{
				...
				tmp.append(String.format("%d&Y=",(messageByte(bits,messageBytes-15) << 24)+(messageByte(bits,messageBytes-16)<<16)+(messageByte(bits,messageBytes-17)<<8)+messageByte(bits,messageBytes-18)));
				tmp.append(String.format("%d'ßFROMÞ/aß ",(messageByte(bits,messageBytes-11) << 24)+(messageByte(bits,messageBytes-12)<<16)+(messageByte(bits,messageBytes-13)<<8)+messageByte(bits,messageBytes-14)));
				tmp.append(" Þa href='http://www.multimap.com/map/browse.cgi?X=");
				tmp.append(String.format("%d&Y=",(messageByte(bits,messageBytes-7) << 24)+(messageByte(bits,messageBytes-8)<<16)+(messageByte(bits,messageBytes-9)<<8)+messageByte(bits,messageBytes-10)));
				tmp.append(String.format("%d'ßDESTÞ/aß ",(messageByte(bits,messageBytes-3) << 24)+(messageByte(bits,messageBytes-4)<<16)+(messageByte(bits,messageBytes-5)<<8)+messageByte(bits,messageBytes-6)));
			}
			...
			if (getViewLinks() == true)	{
				tmp.append(" Þa href='http://www.multimap.com/map/browse.cgi?X=");
				tmp.append(String.format("%d&Y=",(messageByte(bits,messageBytes-7) << 24)+(messageByte(bits,messageBytes-8)<<16)+(messageByte(bits,messageBytes-9)<<8)+messageByte(bits,messageBytes-10)));
				tmp.append(String.format("%d'ßFROMÞ/aß ",(messageByte(bits,messageBytes-3) << 24)+(messageByte(bits,messageBytes-4)<<16)+(messageByte(bits,messageBytes-5)<<8)+messageByte(bits,messageBytes-6)));
			}
	...
	private int headerByte(ExBitSet bits, int num) {
		if (num > 10) System.out.println(String.format("Bad header byte %d !",num));
