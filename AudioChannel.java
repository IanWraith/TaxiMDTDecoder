...
/**
 * @author Andy
 * Abstract class for processing audio
 */
abstract class AudioChannel {
	 ...
	 * @param theApp
	 */
	public AudioChannel(TaxiMdtDecoder theApp){
		this.theApp = theApp;
		packetDecoder = new Decoder(theApp);
	...
	public void processAudio(int bytes, byte[] audioBuffer) {
		...
		theApp.updateVolumeBar(whichChannel, average);
	}
	...
	public void syncHunt(boolean in) {
		...
		if (check_pre == 0xaaaa0000L) {
			...
			if ((high_sync != 0xaa) && (low_sync != 0xaa)) {
				...
				// Display this
				theApp.addLine(line, whichChannel);
				preambleRxed = 0;
			}
		}
	}
	...
	/**
	 * Look for 16 bits of preamble then display the following 64 bits
	...
	public void preambleCheck(boolean in) {
		...
		if (preambleHuntStatus == 1) {
			...
			if (bitsRxed == 64) {
				String dline = "P ";
				for (i = 63; i > -1; i--) {
					if (rawBits.get(i) == true)
						dline = dline + "1";
					else
						dline = dline + "0";
				}
				theApp.addLine(dline, whichChannel);
