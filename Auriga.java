...
public class Auriga {
	...
	public void decode(ExBitSet unstuffedData, SystemInfo si) {
		...
		if ((aurigaCrc.getCrc16Value() != actCrc) && (viewBadCrc == false)) {
		...
		if ((aurigaCrc.getCrc16Value() != actCrc) && (viewBadCrc == true))
			tmp.append(" (BAD CRC !)");
		
		tmp.append(" Auriga ");
		
		if (unstuffedData.get(0) == true){
			tmp.append(" (Base) ");
		}else{
			tmp.append(" (Mobile) ");
		}
		...
		int type = 0;
		if (rdata.get(16) == true) type=type+8;
		if (rdata.get(17) == true) type=type+4;
		if (rdata.get(18) == true) type=type+2;
		if (rdata.get(19) == true) type=type+1;
		tmp.append(" (Type ");
		tmp.append(type);
		...
		if (bcount > 48){
			...
			if (rdata.get(32) == true) stype=stype+128;
			if (rdata.get(33) == true) stype=stype+64;
			if (rdata.get(34) == true) stype=stype+32;
			if (rdata.get(35) == true) stype=stype+16;
			if (rdata.get(36) == true) stype=stype+8;
			if (rdata.get(37) == true) stype=stype+4;
			if (rdata.get(38) == true) stype=stype+2;
			if (rdata.get(39) == true) stype=stype+1;
	...
	private int getCrc(ExBitSet unstuffedData){
		...
		if (unstuffedData.get(bcount - 1) == false)
			crc = 1;
		if (unstuffedData.get(bcount - 2) == false)
			crc = crc + 2;
		if (unstuffedData.get(bcount - 3) == false)
			crc = crc + 4;
		if (unstuffedData.get(bcount - 4) == false)
			crc = crc + 8;
		if (unstuffedData.get(bcount - 5) == false)
			crc = crc + 16;
		if (unstuffedData.get(bcount - 6) == false)
			crc = crc + 32;
		if (unstuffedData.get(bcount - 7) == false)
			crc = crc + 64;
		if (unstuffedData.get(bcount - 8) == false)
			crc = crc + 128;
		if (unstuffedData.get(bcount - 9) == false)
			crc = crc + 256;
		if (unstuffedData.get(bcount - 10) == false)
			crc = crc + 512;
		if (unstuffedData.get(bcount - 11) == false)
			crc = crc + 1024;
		if (unstuffedData.get(bcount - 12) == false)
			crc = crc + 2048;
		if (unstuffedData.get(bcount - 13) == false)
			crc = crc + 4096;
		if (unstuffedData.get(bcount - 14) == false)
			crc = crc + 8192;
		if (unstuffedData.get(bcount - 15) == false)
			crc = crc + 16384;
		if (unstuffedData.get(bcount - 16) == false)
			crc = crc + 32768;
	...
	private int calcSysCode(ExBitSet unstuffedData){
		...
		if (unstuffedData.get(1) == true)
			sysCode = 16384;
		if (unstuffedData.get(2) == true)
			sysCode = sysCode + 8192;
		if (unstuffedData.get(3) == true)
			sysCode = sysCode + 4096;
		if (unstuffedData.get(4) == true)
			sysCode = sysCode + 2048;
		if (unstuffedData.get(5) == true)
			sysCode = sysCode + 1024;
		if (unstuffedData.get(6) == true)
			sysCode = sysCode + 512;
		if (unstuffedData.get(7) == true)
			sysCode = sysCode + 256;
		if (unstuffedData.get(8) == true)
			sysCode = sysCode + 128;
		if (unstuffedData.get(9) == true)
			sysCode = sysCode + 64;
		if (unstuffedData.get(10) == true)
			sysCode = sysCode + 32;
		if (unstuffedData.get(11) == true)
			sysCode = sysCode + 16;
		if (unstuffedData.get(12) == true)
			sysCode = sysCode + 8;
		if (unstuffedData.get(13) == true)
			sysCode = sysCode + 4;
		if (unstuffedData.get(14) == true)
			sysCode = sysCode + 2;
		if (unstuffedData.get(15) == true)
			sysCode++;
	...
	private int calcMobIdent(ExBitSet rdata){
		...
		// ident //
		int mobId = 0;
		if (rdata.get(20) == true)
			mobId = 2048;
		if (rdata.get(21) == true)
			mobId = mobId + 1024;
		if (rdata.get(22) == true)
			mobId = mobId + 512;
		if (rdata.get(23) == true)
			mobId = mobId + 256;
		if (rdata.get(24) == true)
			mobId = mobId + 128;
		if (rdata.get(25) == true)
			mobId = mobId + 64;
		if (rdata.get(26) == true)
			mobId = mobId + 32;
		if (rdata.get(27) == true)
			mobId = mobId + 16;
		if (rdata.get(28) == true)
			mobId = mobId + 8;
		if (rdata.get(29) == true)
			mobId = mobId + 4;
		if (rdata.get(30) == true)
			mobId = mobId + 2;
		if (rdata.get(31) == true)
			mobId++;
	...
	private String returnAurigaCharacter (int start,ExBitSet bits)	{
		...
		if (ascii==163) ret="£";
		else if (ascii==14) ret=" ";
		else ret=Character.toString((char)ascii);
		// Return this
		return ret;
	}
	...
	int i,bpos=0,cpos,dpos,epos,ascii,type,length1,length2,length3,jump;
    // Display the header	
	line[4]="Header : ";
